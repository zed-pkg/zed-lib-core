// Generated by tools/schema-shadow-codegen.mjs. DO NOT EDIT.
// SHADOW ONLY. Production migration authority remains src/rust-orm/sql/registry.sql.
import { pgTable, uuid, text, integer, bigint, boolean, timestamp, jsonb, doublePrecision, primaryKey } from 'drizzle-orm/pg-core';

export const zedApiToken = pgTable("zed_api_tokens", {
  id: uuid("id").notNull().primaryKey(),
  name: text("name").notNull(),
  tokenHash: text("token_hash").notNull(),
  orgId: uuid("org_id"),
  userId: uuid("user_id"),
  role: text("role").notNull(),
  lastUsedAt: timestamp("last_used_at", { withTimezone: true, mode: 'date' }),
  expiresAt: timestamp("expires_at", { withTimezone: true, mode: 'date' }),
  revokedAt: timestamp("revoked_at", { withTimezone: true, mode: 'date' }),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedAuditLog = pgTable("zed_audit_log", {
  id: uuid("id").notNull().primaryKey(),
  orgId: uuid("org_id"),
  actorUserId: uuid("actor_user_id"),
  apiTokenId: uuid("api_token_id"),
  action: text("action").notNull(),
  entityType: text("entity_type").notNull(),
  entityId: uuid("entity_id"),
  detail: jsonb("detail").notNull(),
  clientIpHash: text("client_ip_hash"),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedEntityEmbedding = pgTable("zed_entity_embeddings", {
  id: uuid("id").notNull().primaryKey(),
  entityType: text("entity_type").notNull(),
  entityId: uuid("entity_id").notNull(),
  orgId: uuid("org_id"),
  embeddingModel: text("embedding_model").notNull(),
  embedding: jsonb("embedding").notNull(),
  embeddingDimensions: integer("embedding_dimensions").notNull(),
  contentSha256: text("content_sha256").notNull(),
  contentPreview: text("content_preview"),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedOrg = pgTable("zed_orgs", {
  id: uuid("id").notNull().primaryKey(),
  slug: text("slug").notNull(),
  name: text("name").notNull(),
  description: text("description"),
  settings: jsonb("settings").notNull(),
  createdByUserId: uuid("created_by_user_id"),
  isSoftDeleted: boolean("is_soft_deleted").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedUser = pgTable("zed_users", {
  id: uuid("id").notNull().primaryKey(),
  sharedAuthSubject: uuid("shared_auth_subject").notNull(),
  authRealm: text("auth_realm").notNull(),
  email: text("email"),
  displayName: text("display_name"),
  avatarUrl: text("avatar_url"),
  settings: jsonb("settings").notNull(),
  isSoftDeleted: boolean("is_soft_deleted").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedOrgInvitation = pgTable("zed_org_invitations", {
  id: uuid("id").notNull().primaryKey(),
  orgId: uuid("org_id").notNull().references(() => zedOrg.id),
  invitedByUserId: uuid("invited_by_user_id").notNull(),
  email: text("email").notNull(),
  role: text("role").notNull(),
  tokenHash: text("token_hash").notNull(),
  expiresAt: timestamp("expires_at", { withTimezone: true, mode: 'date' }).notNull(),
  acceptedAt: timestamp("accepted_at", { withTimezone: true, mode: 'date' }),
  acceptedByUserId: uuid("accepted_by_user_id"),
  revokedAt: timestamp("revoked_at", { withTimezone: true, mode: 'date' }),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedOrgMember = pgTable("zed_org_members", {
  orgId: uuid("org_id").notNull().references(() => zedOrg.id),
  userId: uuid("user_id").notNull().references(() => zedUser.id),
  role: text("role").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
}, (table) => [primaryKey({ columns: [table.orgId, table.userId] })]);

export const zedProject = pgTable("zed_projects", {
  id: uuid("id").notNull().primaryKey(),
  orgId: uuid("org_id").notNull().references(() => zedOrg.id),
  slug: text("slug").notNull(),
  name: text("name").notNull(),
  description: text("description"),
  visibility: text("visibility").notNull(),
  settings: jsonb("settings").notNull(),
  createdByUserId: uuid("created_by_user_id"),
  isSoftDeleted: boolean("is_soft_deleted").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedPackage = pgTable("zed_packages", {
  id: uuid("id").notNull().primaryKey(),
  orgId: uuid("org_id").notNull().references(() => zedOrg.id),
  projectId: uuid("project_id").references(() => zedProject.id),
  name: text("name").notNull(),
  description: text("description"),
  visibility: text("visibility").notNull(),
  vcs: text("vcs").notNull(),
  repoUrl: text("repo_url").notNull(),
  homepageUrl: text("homepage_url"),
  keywords: jsonb("keywords").notNull(),
  config: jsonb("config").notNull(),
  downloadCount: bigint("download_count", { mode: 'bigint' }).notNull(),
  versionCount: integer("version_count").notNull(),
  latestVersion: text("latest_version"),
  firstPublishedAt: timestamp("first_published_at", { withTimezone: true, mode: 'date' }),
  visibilityChangedAt: timestamp("visibility_changed_at", { withTimezone: true, mode: 'date' }),
  createdByUserId: uuid("created_by_user_id"),
  isSoftDeleted: boolean("is_soft_deleted").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedProjectInvitation = pgTable("zed_project_invitations", {
  id: uuid("id").notNull().primaryKey(),
  projectId: uuid("project_id").notNull().references(() => zedProject.id),
  invitedByUserId: uuid("invited_by_user_id").notNull(),
  email: text("email").notNull(),
  role: text("role").notNull(),
  tokenHash: text("token_hash").notNull(),
  expiresAt: timestamp("expires_at", { withTimezone: true, mode: 'date' }).notNull(),
  acceptedAt: timestamp("accepted_at", { withTimezone: true, mode: 'date' }),
  acceptedByUserId: uuid("accepted_by_user_id"),
  revokedAt: timestamp("revoked_at", { withTimezone: true, mode: 'date' }),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedProjectMember = pgTable("zed_project_members", {
  projectId: uuid("project_id").notNull().references(() => zedProject.id),
  userId: uuid("user_id").notNull().references(() => zedUser.id),
  role: text("role").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
}, (table) => [primaryKey({ columns: [table.projectId, table.userId] })]);

export const zedPackageDownload = pgTable("zed_package_downloads", {
  id: uuid("id").notNull().primaryKey(),
  packageId: uuid("package_id").notNull().references(() => zedPackage.id),
  packageVersionId: uuid("package_version_id"),
  downloadedByUserId: uuid("downloaded_by_user_id"),
  apiTokenId: uuid("api_token_id"),
  source: text("source").notNull(),
  format: text("format"),
  bytesSent: bigint("bytes_sent", { mode: 'bigint' }),
  clientIpHash: text("client_ip_hash"),
  userAgent: text("user_agent"),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedPackageLicense = pgTable("zed_package_licenses", {
  id: uuid("id").notNull().primaryKey(),
  packageId: uuid("package_id").notNull().references(() => zedPackage.id),
  packageVersionId: uuid("package_version_id"),
  kind: text("kind").notNull(),
  spdxId: text("spdx_id"),
  name: text("name"),
  url: text("url"),
  textBody: text("text_body"),
  isPrimary: boolean("is_primary").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedPackageUpload = pgTable("zed_package_uploads", {
  id: uuid("id").notNull().primaryKey(),
  packageId: uuid("package_id").notNull().references(() => zedPackage.id),
  packageVersionId: uuid("package_version_id"),
  requestedVersion: text("requested_version").notNull(),
  status: text("status").notNull(),
  storageBackend: text("storage_backend").notNull(),
  storageKey: text("storage_key"),
  format: text("format"),
  sizeBytes: bigint("size_bytes", { mode: 'bigint' }),
  sha256: text("sha256"),
  uploadedByUserId: uuid("uploaded_by_user_id"),
  apiTokenId: uuid("api_token_id"),
  clientIpHash: text("client_ip_hash"),
  userAgent: text("user_agent"),
  error: text("error"),
  startedAt: timestamp("started_at", { withTimezone: true, mode: 'date' }).notNull(),
  completedAt: timestamp("completed_at", { withTimezone: true, mode: 'date' }),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedPackageVersion = pgTable("zed_package_versions", {
  id: uuid("id").notNull().primaryKey(),
  packageId: uuid("package_id").notNull().references(() => zedPackage.id),
  version: text("version").notNull(),
  versionScheme: text("version_scheme").notNull(),
  sha256: text("sha256").notNull(),
  sizeBytes: bigint("size_bytes", { mode: 'bigint' }).notNull(),
  format: text("format").notNull(),
  vcsTag: text("vcs_tag"),
  vcsCommit: text("vcs_commit"),
  artifactKey: text("artifact_key").notNull(),
  manifest: jsonb("manifest").notNull(),
  downloadCount: bigint("download_count", { mode: 'bigint' }).notNull(),
  yanked: boolean("yanked").notNull(),
  yankedAt: timestamp("yanked_at", { withTimezone: true, mode: 'date' }),
  yankedReason: text("yanked_reason"),
  publishedByUserId: uuid("published_by_user_id"),
  publishedAt: timestamp("published_at", { withTimezone: true, mode: 'date' }).notNull(),
});

export const zedDependencyGraphArtifact = pgTable("zed_dependency_graph_artifacts", {
  id: uuid("id").notNull().primaryKey(),
  rootPackageVersionId: uuid("root_package_version_id").notNull().references(() => zedPackageVersion.id),
  graphKind: text("graph_kind").notNull(),
  schemaVersion: text("schema_version").notNull(),
  graphDigest: text("graph_digest").notNull(),
  resolverName: text("resolver_name"),
  resolverVersion: text("resolver_version"),
  resolutionInputDigest: text("resolution_input_digest"),
  registryCheckpoint: text("registry_checkpoint"),
  target: jsonb("target").notNull(),
  enabledFeatures: jsonb("enabled_features").notNull(),
  document: jsonb("document").notNull(),
  nodeCount: integer("node_count").notNull(),
  edgeCount: integer("edge_count").notNull(),
  maxDepth: integer("max_depth").notNull(),
  cycleCount: integer("cycle_count").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
  sealedAt: timestamp("sealed_at", { withTimezone: true, mode: 'date' }),
});

export const zedDependencyGraphEdge = pgTable("zed_dependency_graph_edges", {
  id: uuid("id").notNull().primaryKey(),
  graphArtifactId: uuid("graph_artifact_id").notNull().references(() => zedDependencyGraphArtifact.id),
  ordinal: integer("ordinal").notNull(),
  fromRegistryId: text("from_registry_id").notNull(),
  fromOrgSlug: text("from_org_slug").notNull(),
  fromPackageName: text("from_package_name").notNull(),
  fromVersion: text("from_version"),
  fromPackageId: uuid("from_package_id").references(() => zedPackage.id),
  fromPackageVersionId: uuid("from_package_version_id").references(() => zedPackageVersion.id),
  toRegistryId: text("to_registry_id").notNull(),
  toOrgSlug: text("to_org_slug").notNull(),
  toPackageName: text("to_package_name").notNull(),
  toVersion: text("to_version"),
  toPackageId: uuid("to_package_id").references(() => zedPackage.id),
  toPackageVersionId: uuid("to_package_version_id").references(() => zedPackageVersion.id),
  requirement: text("requirement"),
  dependencyKind: text("dependency_kind").notNull(),
  optional: boolean("optional").notNull(),
  defaultFeatures: boolean("default_features").notNull(),
  features: jsonb("features").notNull(),
  target: text("target"),
  minimumDepth: integer("minimum_depth").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: 'date' }).notNull(),
});
